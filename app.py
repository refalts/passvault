...
def register():
    if request.method == 'POST':
        ...
        # Hash password dan generate secret key untuk OTP
        hashed_pw = generate_password_hash(password, method='pbkdf2:sha256')
        ...
        try:
            ...
            cursor.execute(
                'INSERT INTO users (username, password, role, otp_secret, created_at) '
                'VALUES (%s, %s, %s, %s, NOW())',
                (username, hashed_pw, 'user', otp_secret)
            )
            conn.commit()
            flash('Registrasi berhasil! Pindai QR Code untuk mengaktifkan OTP.', 'success')
...
def login():
    if request.method == 'POST':
        ...
        otp_input = request.form.get('otp')  # Ambil input OTP jika ada
        ...
        try:
            ...
            if user:
                ...
                if check_password_hash(user['password'], password):
                    ...
                    # Jika pengguna memiliki OTP aktif
                    if otp_secret:
                        if otp_input:
                            # Verifikasi OTP
                            if pyotp.TOTP(otp_secret).verify(otp_input):
                                # Login berhasil
                                session['user_id'] = user['id']
                                session['username'] = user['username']
                                session['role'] = user['role']
                                flash('Login berhasil!', 'success')
                                return redirect(url_for('form_pengajuan') if user['role'] == 'user' else url_for('admin_dashboard'))
                            else:
                                flash('Kode OTP tidak valid.', 'danger')
                                return render_template('login.html', otp_required=True, username=username)
                        else:
                            # OTP belum diisi
                            flash('Masukkan kode OTP untuk melanjutkan.', 'info')
                            return render_template('login.html', otp_required=True, username=username)
                    else:
                        # Jika OTP tidak diaktifkan, langsung login
                        session['user_id'] = user['id']
                        session['username'] = user['username']
                        session['role'] = user['role']
                        flash('Login berhasil!', 'success')
...
# Generate OTP secret for each user (simpan di database)
...
def generate_qr(username):
    ...
    if not user or not user['otp_secret']:
        flash('Pengguna tidak ditemukan atau OTP secret tidak tersedia.', 'danger')
        return redirect(url_for('login'))

    otp_secret = user['otp_secret']
    otp_uri = pyotp.TOTP(otp_secret).provisioning_uri(
        name=f"{username}@PassVault",
        issuer_name="PassVault | e-passport"
    )
    ...
    buf.seek(0)
    qr_code_image = base64.b64encode(buf.read()).decode('utf-8')  # Convert image to Base64
...
# Tambahkan kolom otp_secret di database user
@app.route('/enable_otp/<username>')
def enable_otp(username):
    ...
    flash('OTP berhasil diaktifkan. Pindai QR Code untuk menghubungkan aplikasi authenticator.', 'success')
    return redirect(url_for('generate_qr', username=username))
